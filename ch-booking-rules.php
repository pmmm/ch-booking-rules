...
class CH_Booking_Rules {
    ...
    public function __construct(){
        ...
        add_action('admin_enqueue_scripts', [$this, 'enqueue_admin_scripts']); // NOVO: Carrega scripts de admin
    }
    ...
    // NOVO: Função para guardar as configurações (suporta JSON ou dados de formulário)
    private function save_settings($cfg){
        $save_success = true;
        
        // 1. Prioridade: Tratar os dados de formulário (Épocas/Promoções)
        if (isset($_POST['seasons']) && is_array($_POST['seasons'])) {
            // Filtrar linhas vazias e re-indexar o array
        ...
        // 2. Fallback: Tratar o JSON da aba Avançado (substitui tudo se for válido)
        if (!empty($_POST['ch_rules_json'])) {
            $raw = wp_unslash($_POST['ch_rules_json']);
            $decoded = json_decode($raw, true);
            if (is_array($decoded)) {
                $cfg = $decoded; // Sobrescreve tudo com o JSON se for válido
                $save_success = true;
            } else {
                $save_success = false;
            }
    ...
    public function settings_page(){
        ...
        // NOVO: Verifica a ação de guardar
        if (isset($_POST['ch_booking_rules_save']) && check_admin_referer('ch_booking_rules_save_nonce')) {
            $cfg = $this->save_settings($cfg);
        }

        $json_raw = json_encode($cfg, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE); // JSON para a aba Avançado
    ...
    // NOVO: Função de enfileiramento de scripts de administração
    public function enqueue_admin_scripts(){
        if ( ! isset( $_GET['page'] ) || $_GET['page'] !== 'ch-booking-rules' ) {
            return;
        }
        
        // Carrega o script que criámos para campos repetíveis
        wp_enqueue_script('ch-admin-js', plugins_url('assets/ch-admin.js', __FILE__), ['jquery'], self::VER, true);
        
        // Adiciona CSS básico inline para formatação da interface
        $css = '.chbr-rule-row { display: flex; gap: 10px; margin-bottom: 10px; align-items: center; } .chbr-rule-row input { padding: 4px; } .chbr-rule-row input[type="number"] { width: 100px; }';
        wp_add_inline_style('wp-admin', $css);
    }
    
    // NOVO: Renderiza o conteúdo da aba Épocas
    public function render_seasons_tab($cfg){
        ...
        // Itera sobre as regras existentes
        ...
        // CAMPO OCULTO (TEMPLATE) PARA O JAVASCRIPT
        echo '<template id="chbr-season-template">';
        $this->render_season_row('{{INDEX}}', $this->default_season_row());
        echo '</template>';
    }

    // NOVO: Função auxiliar para renderizar uma linha de época
    ...
    // NOVO: Configuração por defeito para uma linha de época vazia
    ...
    // NOVO: Placeholder para a aba de Promoções (será desenvolvido a seguir)
    public function render_promos_tab($cfg){
        echo '<h2>'.esc_html__('Códigos Promocionais e Descontos','ch-booking-rules').'</h2>';
        echo '<p class="description">'.esc_html__('Esta secção será desenvolvida a seguir para criar e gerir códigos promocionais de forma simples. Por enquanto, use a aba "Avançado & Debug".','ch-booking-rules').'</p>';
    }
